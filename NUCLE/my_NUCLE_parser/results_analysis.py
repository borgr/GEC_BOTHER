...
def plot_mistakes(all_mistakes, mistakes, ranks, tpe, error="confidence"):
    """
    ...
    if error == "confidence":
        eror_symb = CONFIDENCE
        error_bar = np.array([[mistakes["lower"][mis], mistakes["upper"][mis]] for mis in mistakes.index])
    elif error == "std":
        error_symb = "std"
        error_bar = np.array([[mistakes["weights"][mis] - mistakes["std"][mis], mistakes["weights"][mis] + mistakes["std"][mis]] for mis in mistakes.index])
    else:
        raise ValueError(f"Unknown error bar of type {error} required")

    yerr = np.c_[means - error_bar[:, 0], error_bar[:, 1] - means].T
    ...
    save_to = os.path.join(FIG_DIR, f"{error_symb}_{full_type_name}_weights.png")
    plt.tight_layout()
    plt.savefig(save_to)
    ...
    if error == "confidence":
        eror_symb = CONFIDENCE
        error_bar = np.array([[mistakes["lower_rank"][mis], mistakes["upper_rank"][mis]] for mis in mistakes.index])
    elif error == "std":
        error_symb = "std"
        error_bar = np.array([[mistakes["mean_rank"][mis] - mistakes["std_ranks"][mis], mistakes["mean_rank"][mis] + mistakes["std_ranks"][mis]] for mis in mistakes.index])
    else:
        raise ValueError(f"Unknown error bar of type {error} required")
    ...
    if tpe == AnnotationType.SERCL:
        plt.xticks(fontsize=5)
    plt.tight_layout()
    save_to = os.path.join(FIG_DIR, f"{error_symb}_{full_type_name}_ranks.png")
    plt.savefig(save_to)
    plt.show()
...
def analyze(df, tpe, error, force=True, force_bootstrap=True):
    """
    ...
    mistakes["upper"], mistakes["lower"], mistakes["std"], mistakes["upper_rank"], mistakes["lower_rank"], mistakes["std_ranks"]= \
        all_mistakes.transpose()["upper"], all_mistakes.transpose()["lower"],all_mistakes.transpose()["std"], all_mistakes.transpose()[
            "upper_ranks"], all_mistakes.transpose()["lower_ranks"],  all_mistakes.transpose()["std_ranks"]
    ...
    all_mistakes.drop(["upper", "lower", "std", "lower_ranks", "upper_ranks", "std_ranks","mean_rank"], inplace=True)
    plot_mistakes(all_mistakes, mistakes, ranks, tpe, error)
    return mistakes
